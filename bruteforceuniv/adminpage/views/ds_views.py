...
def ds_points(request):
    ...
    for ds_user in ds_user_list:
        ...
        if not ds_user.weeklyactivitypoints_set.filter(
            week_num=week
        ):  # 처음으로 집계를 누른 상태일 때

            ds_user_study_record = ds_user.weeklystudies_set.filter(
                week=week
            ).first()  # 해당 주차 학습을 했는지
        ...
        ds_user_weekly_studies = (
            ds_user.weeklyactivitypoints_set.all()
        )  # 현재까지 기록된 주차별 학습 활동 점수
        ds_user_progress_record = (
            ds_user.progress_set.all().first()
        )  # 현재까지 기록된 진도율 점수 (유저 당 record 하나)
        ds_user_additional_record = (
            ds_user.additionalpoints_set.all().first()
        ...
        if ds_user_progress_record:  # 진도율 기록이 있으면 계산
            progress_ap = (
                ds_user_progress_record.progress1
                + ds_user_progress_record.progress2
                + ds_user_progress_record.progress3
                + ds_user_progress_record.progress4
            )
        else:
