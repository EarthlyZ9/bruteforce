...
def python_points(request): 
  ...
  for python_user in python_user_list:
    ...
    if not python_user.weeklyactivitypoints_set.filter(week_num=week): #처음으로 집계를 누른 상태일 때

      python_user_study_record = python_user.weeklystudies_set.filter(week=week).first() #해당 주차 학습을 했는지
    ...
    python_user_weekly_studies = python_user.weeklyactivitypoints_set.all() #현재까지 기록된 주차별 학습 활동 점수 
    python_user_progress_record = python_user.progress_set.all().first() #현재까지 기록된 진도율 점수 (유저 당 record 하나)
    python_user_additional_record = python_user.additionalpoints_set.all().first() #현재까지 기록된 추가 점수 (유저 당 record 하나)
    ...
    if python_user_progress_record: #진도율 기록이 있으면 계산
      progress_ap =  python_user_progress_record.progress1 +  python_user_progress_record.progress2 +  python_user_progress_record.progress3 +  python_user_progress_record.progress4
    else:
    ...
    if python_user_additional_record:
      ...
      python_user_additional_record = python_user.additionalpoints_set.all().first()
      additional_ap = python_user_additional_record.additional_points
    ...
    user_points_status = python_user.pointsstatus_set.filter(user=python_user).first()
  ...
  if individual == "true" :
      ...
      python_user_list = Account_Info.objects.filter(course='python', individual=False)
  else:  # all
      python_user_list = Account_Info.objects.filter(course='python').order_by('name')
  ...
  context = {'python_user_list': page_obj, 'page':page,'kw':kw,'individual':individual,'weekly_ap':weekly_ap, 'progress_ap':progress_ap,'additional_ap':additional_ap,'total_ap':total_ap, 'week_list':week_list}
