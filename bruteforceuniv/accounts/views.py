...
def profile(request):
    ...
    week_num = (
        Week.objects.filter(
            season=user_info.season, start_date__lte=date_now, end_date__gte=date_now
        )
        .first()
        .week_num
    )
...
def purchase_records(request):
    ...
    return render(
        request,
        "accounts/purchase_records.html",
        {
            "purchase_records": purchase_records,
            "user_points_status": user_points_status,
        },
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            if Account_Info.objects.filter(name=user_name):  # 등록된 사용자 이름이면!
                ...
                if (
                    user_info.mobile_num == user_mobile_num
                    and user_info.email == user_email
                ):  # 등록된 이름의 휴대폰 번호와 이메일이 일치하면
                    user_id = user_info.username
                    return render(
                        request,
                        "accounts/find_id_complete.html",
        ...
        if form.is_valid():  # valid 시 랜덤 문자열 생성, 그걸로 비번 바꾸기, 이메일 보내기
            ...
            if Account_Info.objects.filter(email=user_email):  # 등록된 이메일이면!
                ...
                html_message = render_to_string(
                    ...
                    {"temporary_password": random_string},
                )
