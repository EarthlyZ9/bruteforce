...
def rankings(request):
    ...
    python_top_ten = PointsStatus.objects.filter(user__course="python").order_by(
        "total_points"
    )[:10]
    ds_top_ten = PointsStatus.objects.filter(user__course="datascience").order_by(
        "total_points"
    )[:10]
    htmlcss_top_ten = PointsStatus.objects.filter(user__course="htmlcss").order_by(
        "total_points"
    )[:10]

    return render(
        request,
        "activities/rankings.html",
        {
            "python_top_ten": python_top_ten,
            "ds_top_ten": ds_top_ten,
            "htmlcss_top_ten": htmlcss_top_ten,
            "week": week,
        },
    )
...
def htmlcss_fruits(request):
    ...
    if kw:
        ...
        html_fruits = html_fruits.filter(
            Q(user__icontains=kw) | Q(topic__icontains=kw)
        ).distinct()
...
class FileDownLoadView(SingleObjectMixin, View):
    ...
    def get(self, request, pk):
        ...
        response[
            "Content-Disposition"
        ] = f"attachment; filename={object.get_filename()}"
...
def weekly_studies(request):
    ...
    if request.method == "POST":
        ...
        if weekly_studies_inst:
            ...
            form = WeeklyStudiesForm(
                updated_request, request.FILES, instance=weekly_studies_inst
            )
        else:
            updated_request = post_data.copy()
            updated_request.update({"user": user, "week": week})
        ...
        return render(
            request,
            "activities/weekly_studies.html",
            {"week": week, "weekly_studies": weekly_studies_inst},
        )
