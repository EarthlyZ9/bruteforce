...
def htmlcss_points(request): 
  ...
  for html_user in html_user_list:
    ...
    if not html_user.weeklyactivitypoints_set.filter(week_num=week): #처음으로 집계를 누른 상태일 때

      html_user_study_record = html_user.weeklystudies_set.filter(week=week).first() #해당 주차 학습을 했는지
    ...
    html_user_weekly_studies = html_user.weeklyactivitypoints_set.all() #현재까지 기록된 주차별 학습 활동 점수 
    html_user_progress_record = html_user.progress_set.all().first() #현재까지 기록된 진도율 점수 (유저 당 record 하나)
    html_user_additional_record = html_user.additionalpoints_set.all().first() #현재까지 기록된 추가 점수 (유저 당 record 하나)
    ...
    if html_user_progress_record: #진도율 기록이 있으면 계산
      progress_ap =  html_user_progress_record.progress1 +  html_user_progress_record.progress2 +  html_user_progress_record.progress3 +  html_user_progress_record.progress4
    else:
  ...
  context = {'html_user_list': page_obj, 'page':page,'kw':kw, 'weekly_ap':weekly_ap, 'progress_ap':progress_ap,'additional_ap':addtional_ap,'total_ap':total_ap, 'week_list':week_list}
